// src/controllers/publicationController.js (versão com menções)

const db = require('../config/database');

// Função auxiliar para processar menções
const processarMencoes = async (texto, pub_id) => {
    ...
    // Usamos um Set para garantir que cada usuário seja mencionado apenas uma vez por post
    const usernames = [...new Set(matches.map(m => m.substring(1)))];

    for (const username of usernames) {
        try {
            // Procura o usuário pelo "username" (a parte antes do @ no e-mail)
...
exports.createPublication = async (req, res) => {
  ...
  try {
    ...
    // Processa as menções DEPOIS de criar a publicação
    if (texto) {
    ...
    res.status(201).json({
      ...
      mencoes: [], // Retorna array vazio pois o feed será recarregado
    });
  } catch (error) {
    console.error(error);
...
exports.getAllPublications = async (req, res) => {
  ...
  try {
    // QUERY ATUALIZADA para incluir as menções de cada post
    const publications = await db.query(
      `SELECT 
         ...
         COALESCE(mentions_agg.mencoes, '[]'::json) as mencoes -- Pega o JSON de menções
       FROM publicacao p 
       JOIN usuario u ON p.autor_id = u.id_usuario
       LEFT JOIN midia m ON p.id_pub = m.pub_id
       -- Subquery para agregar todas as menções de um post num único campo JSON
       LEFT JOIN (
...
// ... as funções updatePublication e deletePublication continuam as mesmas ...
